...
for i in range(0, NUM_MATCHES):
    ...
    randomKeys.add(f"{randomKey:064x}")
...
result = subprocess.run([KEY_SEARCH_PATH, "run", "-s", startKey_hex, "-e", f"{endKey:064x}" ,"-d", DB_FILE_PATH], capture_output=True, text=True, check=True)
print(result.stdout)
print(result.stderr)
#print("Return code:", result.returncode)
